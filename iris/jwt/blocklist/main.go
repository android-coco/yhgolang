...
func main() {
	...
	// http://localhost:8888
	// http://localhost:8888/protected?token=$token
	// http://localhost:8888/logout?token=$token
	// http://localhost:8888
	_ = app.Listen(":8888")
}
func authenticate(ctx iris.Context) {
	...
	_, _ = ctx.HTML(string(token))
}
func protected(ctx iris.Context) {
	claims := jwt.Get(ctx).(*userClaims)
	// To the standard claims, e.g. the generated ID:
	//jwt.GetVerifiedToken(ctx).StandardClaims.ID
	expiresAtString := jwt.GetVerifiedToken(ctx).StandardClaims.ExpiresAt().Format(ctx.Application().ConfigurationReadOnly().GetTimeFormat())
	timeLeft := jwt.GetVerifiedToken(ctx).StandardClaims.Timeleft()
	_, _ = ctx.HTML("%s-----%s----%s----%s", jwt.GetVerifiedToken(ctx).StandardClaims.ID, claims.Username, expiresAtString,timeLeft)
}
